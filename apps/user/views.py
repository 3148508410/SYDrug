...
from authlib.jose import jwt, JoseError  # 生成用户token
from django.contrib.auth import authenticate, login, logout
from utils.mixin import LoginRequiredMixin  # 验证登录，需要验证登录的类视图需要先继承这个类
from django_redis import get_redis_connection
from apps.order.models import OrderInfo, OrderGoods
from django.core.paginator import Paginator


# Create your views here.celery
# /user/register
    ...
    def post(self, request):
        ...
        if user is not None:
            ...
            if user.is_active:
                ...
                response = redirect(reverse('goods:index'))  # redirect返回的是一个HttpResponseRedirect对象
                login(request, user)
                ...
                if remember == 'on':
                    # 记住用户名
                    response.set_cookie('username', username, max_age=7 * 24 * 3600)  # 设置过期时间为一周，单位为秒
                else:
                    # 未勾选则删除cookies
